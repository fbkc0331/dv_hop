...
# import pexpect            #todo : 이 module은 현재 쓸 계획 X
...
# import queue              # todo : 왜인지 Rpi에서 실행이 안됨
...
# mac address of Rpi. A0, A1, A2, A3, N1, N2, ... , N26 in order
...
# function : determine host_ID to send to server based on hostname of Rpi
def comfirm_host_ID():
...
# funcion : scan ble devices and enqueue raw data(iBeacon packet)
...
# function : send data form "host_ID,Rpi_ID,rssi" to server
def send_ble_data_to_server(q, host_ID):
    client_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

    # wait for socket connect
    while 1:
        if client_sock.connect_ex((SERVER, PORT)) != 0:
            break
        ...
        if q.empty() is False:
            ...
            for ble_data in ble_raw_data:
                ...
                if int(rssi) > RSSI_THRESHOLD:
                    Rpi_ID = confirm_Rpi_ID(mac_address)                                   # find device number of scanned device
                    if Rpi_ID is not False:
                        send_data = str(host_ID) + ',' + str(Rpi_ID) + ',' + str(rssi)     # make data form "host_ID,Rpi_ID,rssi"
